...
<title></title>
<script type="text/javascript" src="/staticfile/js/jquery-1.12.4.js"></script>
<style>
textarea {
	height: 300px;
	width: 100%;
...
<script>
		...
		String basePath2 = request.getScheme() + "://"
			...
			+ path + "/";*/
		var path = '<%=basePath%>';
		var uid="${messageUser.getId() eq null?-1:messageUser.getId()}";
		if(uid==-1){
			location.href="<%=basePath2%>";}

		var toName = '${messageToUser.getName()}';
		var fromId = uid;					//发送者的id
		var fromName='${messageUser.getName()}';		//发送者姓名
		//var toId=uid==1?2:1;       //接受对象这里需要改成动态获取
		var toId = '${messageToUser.getId()}';
		var websocket;
...
<body>
	和<a href="#" style="color: crimson">${messageToUser.getName() }</a>聊天中:
	<div id="content"></div>
	<input type="text" placeholder="请输入要发送的信息" id="msg" class="msg" onkeydown="send(event)">
	<input type="button" value="聊天记录" class="old" onclick="oldMsg()" >
